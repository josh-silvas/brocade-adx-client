...
func New(a adc.ADCSoapClient) NET {
	return NET{
		adc: a,
	}
}
...
func (s NET) Net(method string) (*NET, int, error) {
...
// GetInterfaceConfig method will display and configure basic system management functions on the
// ServerIron ADX device. Initialize with an ADX struct, then call with a sys call.
func (s NET) GetInterfaceConfig(port, interfaceType string) (*NET, int, error) {
	it := strings.ToUpper(interfaceType)
	...
	payload := soap.XMLMarshalHead(g)
	if payload == "" {
		return nil, 0, errors.New("Unable to successfully marshal a XML request")
	}
	resp, code, err := soap.XMLADXBasicAuthPost(s.adc.URL+"/WS/NET", "getInterfaceConfig", payload, s.adc.User, s.adc.Passwd)
	if err != nil {
		return nil, code, fmt.Errorf("XMLADXBasicAuthPost Error: %s", err.Error())
	}
	e, err := xmlUnmarshal(resp)
...
func xmlUnmarshal(resp []byte) (*NET, error) {
	var e NET
